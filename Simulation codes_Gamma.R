...
for(i in 1:SIMULATION_N){
  ...
  #Y1
  temp_gamma_mean=2*EFFECT_STAGE_I_SD # "2 sigma rule" to mimic the variability in a normal distribution with sd = EFFECT_STAGE_I_SD
  gamma_rate = temp_gamma_mean/EFFECT_STAGE_I_SD^2
  gamma_shape = temp_gamma_mean*gamma_rate
  
  P <- rgamma(SAMPLE_SIZE_ARM_P, shape=gamma_shape, rate = gamma_rate) + EFFECT_STAGE_I_P - temp_gamma_mean
  L <- rgamma(SAMPLE_SIZE_ARM_L, shape=gamma_shape, rate = gamma_rate) + EFFECT_STAGE_I_L - temp_gamma_mean
  H <- rgamma(SAMPLE_SIZE_ARM_H, shape=gamma_shape, rate = gamma_rate) + EFFECT_STAGE_I_H - temp_gamma_mean
  ...
  #Y2
  mu2 <- EFFECT_STAGE_I[DATA_TREAT_STAGE_II] + ALPHA + BETA * (Y1-EFFECT_STAGE_I[DATA_TREAT_STAGE_I])
  Y2 <- rgamma(SAMPLE_SIZE_TOTAL, shape=gamma_shape, rate = gamma_rate) + mu2 - temp_gamma_mean
...
bias_discount_parameter_posterior_mean=colMeans(discount_parameter_posterior_mean)-c(ALPHA, BETA)
